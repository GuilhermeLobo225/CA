"""SmartHandover - End-to-End Inference Pipeline v5.
...
audio (16 kHz mono) ->
  ...
  -> meta_classifier_v5_calibrated.pkl (LR + isotonic, 5-class)
  -> handover_score = w_anger * P(anger) + w_frust * P(frust)
  -> compare to threshold (handover_threshold_v5.json)
...
# v5 feature schema (20 dims: 5 RoBERTa + 6 GoEmo + 4 VADER + 5 audio)
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "meta_checkpoint":    os.path.join(CKPT_DIR,
                                          "meta_classifier_v5_calibrated.pkl"),
    "threshold_json":     os.path.join(CONFIG_DIR,
                                          "handover_threshold_v5.json"),
    "device":             None,
    "max_text_length":    128,
...
class SmartHandoverPipelineV5:
    """v5 audio -> handover decision pipeline (best-of state).

    Differences vs v1:
      * RoBERTa: ``roberta_combined.pt`` (multi-corpus, MELD + synth).
      * Audio: ``wav2vec2_finetuned.pt`` (Day F13, 5 native classes).
      * Meta: ``meta_classifier_v5_calibrated.pkl`` (LR + isotonic, 20-dim).
      * Handover: weighted score (w_anger * P(anger) + w_frust * P(frust))
                  vs single threshold ``configs/handover_threshold_v5.json``.
    """
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.device = device
        print(f"[SmartHandoverPipelineV5] Initialising on device={device}")
        ...
        # --- 6. Meta-classifier v5 ---
        print(f"  loading meta v5: {cfg['meta_checkpoint']}")
        ...
        print("[SmartHandoverPipelineV5] Ready.\n")
    ...
    def predict_from_audio(
        ...
        timings["audio_v5"] = time.perf_counter() - t
        ...
        return {
            ...
            "details": {
                ...
                "audio_v5":       audio_scores,  # 5 native classes
            },
        }
